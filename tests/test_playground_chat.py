...
def test_playground_hides_email_card_and_passport() -> None:
    chat = PlaygroundChat(AISafetyGateway(vault=InMemorySessionVault()))
    turn = chat.chat(
        "web-3",
    ...
    assert "ming@school.edu.tw" not in turn.sent_to_llm
    assert "4111111111111111" not in turn.sent_to_llm
    assert "A12345678" not in turn.sent_to_llm
    assert "<EMAIL_ADDRESS_1>" in turn.sent_to_llm
    assert "<CREDIT_CARD_1>" in turn.sent_to_llm
    assert "<TW_PASSPORT_1>" in turn.sent_to_llm
    assert "ming@school.edu.tw" in turn.child_reply
    assert turn.pii_leaked_to_llm is False


def test_playground_treats_gemini_safety_block_as_child_turn() -> None:
    ...
    chat = PlaygroundChat(AISafetyGateway(vault=InMemorySessionVault()))
    turn = chat.chat("web-4", "我叫王小明，可以教我寫日記嗎？", PolicyLlm())
    assert turn.blocked is True
    assert turn.block_category == "llm_safety"
    assert turn.llm_backend == "blocked"
    assert "不適合" in turn.child_reply
