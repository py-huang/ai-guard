...
class AISafetyGateway:
    ...
    Default chain:
      inbound  : TaiwanPII -> ContentSafety(stub) -> SocraticPedagogy(stub)
      outbound : ContentSafety(stub) -> ParentAudit(stub) -> TaiwanPII (deanonymize last)
    """
    ...
    def __init__(
        ...
        self._inbound_steps: tuple[BaseGuardStep, ...] = tuple(
            ...
            else (pii, ContentSafetyStep(), SocraticPedagogyStep())
        )
        self._outbound_steps: tuple[BaseGuardStep, ...] = tuple(
            outbound_steps
            if outbound_steps is not None
            else (ContentSafetyStep(), ParentAuditStep(), pii)
        )
...
def _find_pii_step(steps: Sequence[BaseGuardStep] | None) -> TaiwanPIIGuardStep | None:
    if not steps:
        return None
    for step in steps:
        if isinstance(step, TaiwanPIIGuardStep):
            return step
    return None
